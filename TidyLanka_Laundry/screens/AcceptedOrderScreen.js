...
import OrderRowAccepted from '../components/OrderRowAccepted';
...
export default function CurrentOrderScreen() {
    ...
    const getOrder =async  () =>{
        ...
        await
        axios
        .get(`http://${window.ip}:5000/order/laundryAccepted/${id}`).then((res)=>{
            setOrders(res.data)
        ...
        <>
            ...
            <ScrollView  >
                ...
                <View
                    ...
                    <View className="px-5">
                        <Text className="text-3xl font-bold">Accepted Orders</Text>
   ...
   {orders.map((order)=>(
    <OrderRowAccepted item={order} />
                        ...
                        //        axios 
                        //        .put(`http://${window.ip}:5000/order/completeOrder/${order._id}`).then(()=>{
                        //            Alert.alert('Order Complted') ;
                        //            navigation.navigate('CompletedOrder') ;
                        //        }).catch((err)=>{
                        //            console.log(err) ;
                        //        })
                        //    }}>
                        //        <Text style={style.deleteText}>Completed</Text>
                        //    </TouchableOpacity>
                        //    </View>
                   ))}
       </ScrollView>
